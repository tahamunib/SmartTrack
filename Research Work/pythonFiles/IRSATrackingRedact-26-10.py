...
def func(roi_histogram,roiBox,term_criteria):
	...
	try:
		...
		if startSec < 0 or endSec < 0 or endSec > totalSec: 				# If Start or End is negative or End is beyond the total then full video should be redacted. 
			startSec = 0
			endSec = totalSec
		if endSec < startSec or startSec >= endSec:							# If End is less than Start or Start is greatar than End then break
			cap.release()
			sys.exit('Invalid Start or End Limit, Exiting..')
		if blurrValue < 0 or blurrValue > 100:								# Blur value should be in 0-100 inclusive.
			cap.release()
